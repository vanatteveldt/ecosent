...
perf = inner_join(scores, gold) %>% 
  ...
  summarize(acc=mean(correct), cor=cor(gold, value), alpha=alpha(gold, value), 
...
```{r}
wide_scores = scores %>% mutate(value=trichotomize(value), 
...
```{r}
varnames = purrr::map(sections, ~filter(names, section == .))
